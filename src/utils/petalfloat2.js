/* eslint-disable camelcase */
/* eslint-disable padded-blocks */
/* eslint-disable no-useless-return */
/* eslint-disable no-redeclare */
/* eslint-disable no-useless-constructor */
class Point {
  constructor () {
    this._win = {
      w: window.innerWidth,
      h: window.innerHeight
    }
    if (Math.random() >= 0.5) {
      var px = 0;
      var py = Math.floor(Math.random() * this._win.h)
    } else {
      var py = 0;
      var px = Math.floor(Math.random() * this._win.w)
    }
    this.x = px;
    this.y = py;
    this._alpha = 0.8;
    this._rotate = 0;
    this._scale = Math.random() * 0.5 + 0.5;
    this.vx = Math.random() * 2 + 0.2;
    this.vy = Math.random() + 0.2;
    this.va = -0.0002;
    this.vr = -(Math.random() * 1 + 1);
    this.life = Math.floor(Math.random() * 200) + 800;
  }

  isDeath () {
    return this.life <= 0;
  }
  Run () {
    this.x += this.vx;
    this.y += this.vy;
    this._rotate += this.vr;
    this._alpha += this.va;
    this.life--;
  }
}

class PF {
  constructor (option) {
    if (!option) { option = {}; }
    this._plist = [];
    // ======================================
    this._pt = Point;
    this._res = 'data:image/png;base64,';
    this._img = null;
    this._win = {
      w: window.innerWidth,
      h: window.innerHeight
    }
    this._index = option.index || 999999;
    this.box = null;
    this.stage = null;
    this._anmition = null;
    this._time = null;
    this._maxtime = option.life || 30;
    this._parent = document.getElementById(option.el) || document.body;
  }
  // ========================================
  init () {
    console.log('PF INIT!')
    if (this.box) return;
    this._img = document.createElement('img');
    this._img.src = this._res;
    this.box = document.createElement('canvas');
    this.box.width = this._win.w;
    this.box.height = this._win.h;
    this.box.style.cssText = 'display: block;position: fixed;top: 0;left: 0;margin: 0;padding: 0;border: none;background: none;pointer-events: none;';
    this.box.style.zIndex = this._index;
    this._parent.appendChild(this.box);
    this.stage = this.box.getContext('2d');
  }
  destory () {
    this.stop();
    this.box.parentElement.removeChild(this.box);
    this.box = null;
  }
  start () {
    console.log('PF START!')
    for (var i = 0; i < 4; i++) {
      this.addImg();
    }

    this.render(1);
  }
  stop () {
    cancelAnimationFrame(this._anmition);
  }
  // =======================================
  addImg () {
    this._plist.push(new Point())
  }
  action () {
    for (let i = 0; i < this._plist.length; i++) {
      this._plist[i].Run();
      if (this._plist[i].isDeath()) {
        this._plist.splice(i, 1);
        i--;
      }
    }
  }
  render (k) {
    this.action()
    if (this._time++ >= this._maxtime) {
      this._time = 0;
      this.addImg();
      this.addImg();
    }

    this.stage.clearRect(0, 0, this.box.width, this.box.height);

    for (let i = 0; i < this._plist.length; i++) {
      this.drawImg(this._plist[i])
    }

    if (k) {
      this._anmition = requestAnimationFrame(() => {
        this.render(1);
      });
    }
  }
  drawImg (pt) {
    var img_w = this._img.width * pt._scale;
    var img_h = this._img.height * pt._scale;

    this.stage.translate(pt.x + img_w / 2, pt.y + img_h / 2);
    this.stage.globalAlpha = pt._alpha;
    this.stage.rotate(-(Math.PI / 180 * pt._rotate));
    this.stage.drawImage(this._img, 0 - img_w, 0 - img_h, img_w, img_h);

    this.stage.rotate(Math.PI / 180 * pt._rotate);
    this.stage.globalAlpha = 1;
    this.stage.translate(-(pt.x + img_w / 2), -(pt.y + img_h / 2));
  }
}

export default PF;